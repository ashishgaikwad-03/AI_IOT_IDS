...
export default function App() {
  ...
  const API_BASE = window.location.port === '5173'
    ? 'http://localhost:8000'  // Vite dev: proxy to local backend
  ...
  return (
    <div className="min-h-screen bg-soc-bg text-soc-text font-sans">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <div className="relative w-10 h-10">
              <svg viewBox="0 0 40 40" className="w-full h-full">
                <polygon points="20,3 37,13 37,27 20,37 3,27 3,13" fill="none" stroke="#00d4ff" strokeWidth="1.5" opacity="0.7" />
                <polygon points="20,8 32,15 32,25 20,32 8,25 8,15" fill="none" stroke="#00d4ff" strokeWidth="1" opacity="0.4" />
                <text x="20" y="23" textAnchor="middle" fill="#00d4ff" fontSize="10" fontWeight="bold" fontFamily="JetBrains Mono">IDS</text>
              </svg>
              {/* Animated pulse ring */}
            ...
            <div>
              <h1 className="text-base font-bold tracking-wider text-soc-text glow-cyan">
      ...
      <footer className="border-t border-soc-border py-2 px-6 flex items-center justify-between text-xs text-soc-muted font-mono">
        <span>FastAPI WebSocket :8000 → TShark Live Capture → XGBoost (11-class) → React</span>
        <span>Software-Only IDS · No Hardware Required · GitHub Model 99.7% F1</span>
        <span>theabrahamaudu/ids_project · XGBoost · TShark</span>
      </footer>
    </div>
  );
}
